...
function calcularSecaoCabo(correnteProjeto) {
    ...
    const resultadoTexto = `<p class="Resultado">Corrente de Projeto :<info>${correnteProjeto}A</info></p><p class="Resultado"> Metodo de Instalação :<info>${metodoInstalacao.value}</info></p><p class="Resultado">Material Condutor :<info>${materialCondutorSelecionado.value}</info></p><p class="Resultado">Material de Isolação :<info>${materialIsolacao.value}</info></p><p class="Resultado">Numero de Condutores Carregados :<info>${numeroCondutores.value.substring(0, 1)}</info></p><p class="Resultado">Para temperatura do<info> ${localInstalacaoAmb.value} </info>de <info>${correcaoTemp.value}ºC</info> valor de :<info> ${valorCorrecao} </info></p><p class="Resultado">Fator de agrupamento de circuitos :<info> ${fatorDeCorrecao} </info></p><p class="Resultado">Seção reta do Cabo :<info> ${result}mm²</info></p>`;
